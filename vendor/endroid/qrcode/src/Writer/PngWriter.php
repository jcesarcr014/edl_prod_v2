...
class PngWriter extends AbstractBaconWriter
{
    ...
    protected function addMargin($sourceImage, $margin, $size, array $foregroundColor, array $backgroundColor)
    {
        ...
        // Aplicamos (int) a todo el cálculo de la dimensión
        $targetSize = (int) ($size + $margin * 2);
        ...
        // Aplicamos (int) a todos los parámetros numéricos para asegurar compatibilidad
        imagecopyresampled(
            $targetImage, 
            $sourceImage, 
            (int)$margin, 
            (int)$margin, 
            (int)$additionalWhitespace, 
            (int)$additionalWhitespace, 
            (int)$size, 
            (int)$size, 
            (int)($size - 2 * $additionalWhitespace), 
            (int)($size - 2 * $additionalWhitespace)
        );
