    ...
    function saveX509($cert, $format = self::FORMAT_PEM)
    {
        ...
        switch (true) {
            ...
            default:
                switch ($algorithm) {
                    case 'rsaEncryption':
                        $cert['tbsCertificate']['subjectPublicKeyInfo']['subjectPublicKey'] = Base64::encode(' ' . Base64::decode(preg_replace('#-.+-|[\\r\\n]#', '', $cert['tbsCertificate']['subjectPublicKeyInfo']['subjectPublicKey'])));
                    ...
                    case 'id-ce-authorityKeyIdentifier':
                        ...
                        if (isset($value['authorityCertSerialNumber'])) {
                            if ($value['authorityCertSerialNumber']->toBytes() == '') {
                                $temp = chr(ASN1::CLASS_CONTEXT_SPECIFIC << 6 | 2) . ' ';
    ...
    function validateSignature($caonly = true)
    {
        ...
        switch (true) {
            case isset($this->currentCert['tbsCertificate']):
                ...
                return $this->_validateSignature($signingCert['tbsCertificate']['subjectPublicKeyInfo']['algorithm']['algorithm'], $signingCert['tbsCertificate']['subjectPublicKeyInfo']['subjectPublicKey'], $this->currentCert['signatureAlgorithm']['algorithm'], substr($this->currentCert['signature'], 1), $this->signatureSubject);
            case isset($this->currentCert['certificationRequestInfo']):
                return $this->_validateSignature($this->currentCert['certificationRequestInfo']['subjectPKInfo']['algorithm']['algorithm'], $this->currentCert['certificationRequestInfo']['subjectPKInfo']['subjectPublicKey'], $this->currentCert['signatureAlgorithm']['algorithm'], substr($this->currentCert['signature'], 1), $this->signatureSubject);
            case isset($this->currentCert['publicKeyAndChallenge']):
                return $this->_validateSignature($this->currentCert['publicKeyAndChallenge']['spki']['algorithm']['algorithm'], $this->currentCert['publicKeyAndChallenge']['spki']['subjectPublicKey'], $this->currentCert['signatureAlgorithm']['algorithm'], substr($this->currentCert['signature'], 1), $this->signatureSubject);
...
' . chunk_split(Base64::encode(substr($key, 1)), 64) . '-----END RSA PUBLIC KEY-----';
            default:
                return $key;
        }
    }
    ...
    function saveCSR($csr, $format = self::FORMAT_PEM)
    {
        ...
        switch (true) {
            ...
            default:
                switch ($algorithm) {
                    case 'rsaEncryption':
                        $csr['certificationRequestInfo']['subjectPKInfo']['subjectPublicKey'] = Base64::encode(' ' . Base64::decode(preg_replace('#-.+-|[\\r\\n]#', '', $csr['certificationRequestInfo']['subjectPKInfo']['subjectPublicKey'])));
    ...
    function saveSPKAC($spkac, $format = self::FORMAT_PEM)
    {
        ...
        switch (true) {
            ...
            default:
                switch ($algorithm) {
                    case 'rsaEncryption':
                        $spkac['publicKeyAndChallenge']['spki']['subjectPublicKey'] = Base64::encode(' ' . Base64::decode(preg_replace('#-.+-|[\\r\\n]#', '', $spkac['publicKeyAndChallenge']['spki']['subjectPublicKey'])));
                }
        }
...
' . chunk_split(Base64::encode($crl), 64) . '-----END X509 CRL-----';
    ...
    function sign($issuer, $subject, $signatureAlgorithm = 'sha256WithRSAEncryption')
    {
        ...
        if (isset($subject->currentCert) && is_array($subject->currentCert) && isset($subject->currentCert['tbsCertificate'])) {
            ...
            /* "The serial number MUST be a positive integer"
               ...
               */
            $serialNumber = !empty($this->serialNumber) ? $this->serialNumber : new BigInteger(Random::string(20) & '' . str_repeat('�', 19), 256);
        ...
        if ($key instanceof RSA) {
            switch ($signatureAlgorithm) {
                ...
                case 'sha512WithRSAEncryption':
                    ...
                    $this->currentCert['signature'] = ' ' . $key->sign($this->signatureSubject, RSA::PADDING_PKCS1);
    ...
    function _formatSubjectPublicKey()
    {
        if ($this->publicKey instanceof RSA) {
            ...
            return array('algorithm' => array('algorithm' => 'rsaEncryption'), 'subjectPublicKey' => $this->publicKey->getPublicKey('PKCS1'));
        }
        return false;
    }
    /**
