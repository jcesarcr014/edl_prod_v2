...
class ASN1
{
     ...
     * @access public
     */
    const STRING_TYPE_SIZE = array(self::TYPE_UTF8_STRING => 0, self::TYPE_BMP_STRING => 2, self::TYPE_UNIVERSAL_STRING => 4, self::TYPE_PRINTABLE_STRING => 1, self::TYPE_TELETEX_STRING => 1, self::TYPE_IA5_STRING => 1, self::TYPE_VISIBLE_STRING => 1);
    /**
     * Parse BER-encoding
     *
    ...
    static function decodeBER($encoded)
    {
        ...
        $decoded = array(self::_decode_ber($encoded));
    ...
    static function _decode_ber($encoded, $start = 0, $encoded_pos = 0)
    {
        ...
        if ($tag == 31) {
            ...
            do {
                ...
                $tag <<= 7;
                $tag |= ord($encoded[$encoded_pos++]) & 127;
                $start++;
            } while ($loop);
        }
        ...
        switch ($class) {
            ...
            case self::CLASS_CONTEXT_SPECIFIC:
                if (!$constructed) {
                    return array('type' => $class, 'constant' => $tag, 'content' => $content, 'length' => $length + $start - $current['start']) + $current;
                }
                $newcontent = array();
        ...
        switch ($tag) {
            ...
            case self::TYPE_SET:
                ...
                while ($content_pos < $content_len) {
                    ...
                    if (!isset($current['headerlength']) && substr($content, $content_pos, 2) == '  ') {
                        $length = $offset + 2;
                        // +2 for the EOC
                        break 2;
                    }
     ...
     * @access public
     */
    static function asn1map($decoded, $mapping, $special = array())
    {
        ...
        switch (true) {
            case $mapping['type'] == self::TYPE_ANY:
                ...
                if (isset($decoded['constant']) || !array_key_exists($intype, self::ANY_MAP) || ord(self::$encoded[$decoded['start']]) & 32) {
                    return new Element(substr(self::$encoded, $decoded['start'], $decoded['length']));
                }
                $inmap = self::ANY_MAP[$intype];
                if (is_string($inmap)) {
                    return array($inmap => self::asn1map($decoded, array('type' => $intype) + $mapping, $special));
                }
                break;
     ...
     * @access public
     */
    static function encodeDER($source, $mapping, $special = array())
    {
        self::$location = array();
     ...
     * @access private
     */
    static function _encode_der($source, $mapping, $idx = null, $special = array())
    {
        ...
        switch ($tag) {
                    ...
                    if (isset($child['constant'])) {
                        ...
                        */
                        if (isset($child['explicit']) || $child['type'] == self::TYPE_CHOICE) {
                            $subtag = chr(self::CLASS_CONTEXT_SPECIFIC << 6 | 32 | $child['constant']);
                            $temp = $subtag . self::encodeLength(strlen($temp)) . $temp;
                        } else {
                            $subtag = chr(self::CLASS_CONTEXT_SPECIFIC << 6 | ord($temp[0]) & 32 | $child['constant']);
            ...
            case self::TYPE_CHOICE:
                ...
                if ($temp && isset($mapping['cast'])) {
                    $temp[0] = chr($mapping['class'] << 6 | $tag & 32 | $mapping['cast']);
                }
                return $temp;
            case self::TYPE_INTEGER:
            case self::TYPE_ENUMERATED:
            ...
            case self::TYPE_OBJECT_IDENTIFIER:
                ...
                for ($i = 2; $i < count($parts); $i++) {
                    ...
                    if (!$parts[$i]) {
                        ...
                        while ($parts[$i]) {
                            $temp = chr(128 | $parts[$i] & 127) . $temp;
                            $parts[$i] >>= 7;
                        }
                        $temp[strlen($temp) - 1] = $temp[strlen($temp) - 1] & chr(127);
                    }
                    $value .= $temp;
                }
                break;
            case self::TYPE_ANY:
                ...
                switch (true) {
                    case !isset($source):
                        return self::_encode_der(null, array('type' => self::TYPE_NULL) + $mapping, null, $special);
                    case is_int($source):
                    case $source instanceof BigInteger:
                        return self::_encode_der($source, array('type' => self::TYPE_INTEGER) + $mapping, null, $special);
                    case is_float($source):
                        return self::_encode_der($source, array('type' => self::TYPE_REAL) + $mapping, null, $special);
                    case is_bool($source):
                        return self::_encode_der($source, array('type' => self::TYPE_BOOLEAN) + $mapping, null, $special);
                    case is_array($source) && count($source) == 1:
                        $typename = implode('', array_keys($source));
                        $outtype = array_search($typename, self::ANY_MAP, true);
                        if ($outtype !== false) {
                            return self::_encode_der($source[$typename], array('type' => $outtype) + $mapping, null, $special);
                        }
                }
        ...
        if (isset($mapping['cast'])) {
            if (isset($mapping['explicit']) || $mapping['type'] == self::TYPE_CHOICE) {
                ...
                $tag = $mapping['class'] << 6 | 32 | $mapping['cast'];
            } else {
                $tag = $mapping['class'] << 6 | ord($temp[0]) & 32 | $mapping['cast'];
            }
        }
        ...
        for ($i = 0; $i < $inlength;) {
            ...
            switch (true) {
                ...
                default:
                    $bit = 6;
                    do {
                        if ($bit > 25 || $i >= $inlength || (ord($in[$i]) & 192) != 128) {
                            return false;
                        }
                        $c = $c << 6 | ord($in[$i++]) & 63;
                        $bit += 5;
