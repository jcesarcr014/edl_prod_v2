...
');
    ...
    function startSubsystem($subsystem)
    {
        ...
        $packet = pack('CNa*N3', NET_SSH2_MSG_CHANNEL_OPEN, strlen('session'), 'session', self::CHANNEL_SUBSYSTEM, $this->window_size, 16384);
        if (!$this->_send_binary_packet($packet)) {
            return false;
        }
        ...
        switch ($this->signature_format) {
            ...
            case 'ssh-rsa':
                ...
                */
                ...
                $s = $s->modPow($e, $n);
                $s = $s->toBytes();
                $h = pack('N4H*', 3154224, 151389483, 235078170, 83887124, sha1($this->exchange_hash));
                $h = chr(1) . str_repeat(chr(255), $nLength - 2 - strlen($h)) . $h;
                if ($s != $h) {
                    //user_error('Bad server signature');
                    return $this->_disconnect(NET_SSH2_DISCONNECT_HOST_KEY_NOT_VERIFIABLE);
                }
