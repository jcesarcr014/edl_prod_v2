...
class PKCS8 extends PKCS
{
                ...
                case 'id-PBES2':
                    ...
                    switch ($keyDerivationFunc['algorithm']) {
                        case 'id-PBKDF2':
                            ...
                            call_user_func_array(array($cipher, 'setPassword'), $params);
                            $key = $cipher->decrypt($decrypted['encryptedData']);
                            $decoded = ASN1::decodeBER($key);
                            if (empty($decoded)) {
                                return false;
    ...
    {
        self::_initialize_static_variables();
        $key = array('version' => 'v1', 'privateKeyAlgorithm' => array('algorithm' => $algorithm), 'privateKey' => $key);
        if (!empty($attr)) {
            $key['attributes'] = $attr;
        }
        ...
        if (!empty($password) && is_string($password)) {
            ...
            if (self::$defaultEncryptionAlgorithm == 'id-PBES2') {
                ...
                $PBKDF2params = array('salt' => $salt, 'iterationCount' => $iterationCount, 'prf' => array('algorithm' => self::$defaultPRF, 'parameters' => null));
                $PBKDF2params = ASN1::encodeDER($PBKDF2params, Maps\PBKDF2params::MAP);
                if (!$crypto instanceof RC2) {
                    $params = array('octetString' => $iv);
                } else {
                    $params = array('rc2ParametersVersion' => 58, 'iv' => $iv);
                    $params = ASN1::encodeDER($params, Maps\RC2CBCParameter::MAP);
                    $params = new ASN1\Element($params);
                }
                $params = array('keyDerivationFunc' => array('algorithm' => 'id-PBKDF2', 'parameters' => new ASN1\Element($PBKDF2params)), 'encryptionScheme' => array('algorithm' => self::$defaultEncryptionScheme, 'parameters' => $params));
                ...
                $params = array('salt' => $salt, 'iterationCount' => $iterationCount);
                $params = ASN1::encodeDER($params, Maps\PBEParameter::MAP);
            }
            $crypto->setPassword($password, $kdf, $hash, $salt, $iterationCount);
            $key = $crypto->encrypt($key);
            $key = array('encryptionAlgorithm' => array('algorithm' => self::$defaultEncryptionAlgorithm, 'parameters' => new ASN1\Element($params)), 'encryptedData' => $key);
...
' . chunk_split(Base64::encode($key), 64) . '-----END PRIVATE KEY-----';
    ...
    {
        self::_initialize_static_variables();
        $key = array('publicKeyAlgorithm' => array('algorithm' => $algorithm, 'parameters' => null), 'publicKey' => ' ' . $key);
