...
class PKCS1 extends Progenitor
{
    ...
    static function load($key, $password = '')
    {
        ...
        if (is_array($key)) {
            $components += array('modulus' => $key['modulus'], 'publicExponent' => $key['publicExponent'], 'privateExponent' => $key['privateExponent'], 'primes' => array(1 => $key['prime1'], $key['prime2']), 'exponents' => array(1 => $key['exponent1'], $key['exponent2']), 'coefficients' => array(2 => $key['coefficient']));
    ...
    static function savePrivateKey(BigInteger $n, BigInteger $e, BigInteger $d, $primes, $exponents, $coefficients, $password = '')
    {
        ...
        $key = array('version' => $num_primes == 2 ? 'two-prime' : 'multi', 'modulus' => $n, 'publicExponent' => $e, 'privateExponent' => $d, 'prime1' => $primes[1], 'prime2' => $primes[2], 'exponent1' => $exponents[1], 'exponent2' => $exponents[2], 'coefficient' => $coefficients[2]);
        for ($i = 3; $i <= $num_primes; $i++) {
            $key['otherPrimeInfos'][] = array('prime' => $primes[$i], 'exponent' => $exponents[$i], 'coefficient' => $coefficients[$i]);
        }
