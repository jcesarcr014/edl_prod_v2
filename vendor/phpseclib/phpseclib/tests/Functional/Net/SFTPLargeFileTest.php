...
class Functional_Net_SFTPLargeFileTest extends Functional_Net_SFTPTestCase
{
    ...
    public function testPutSizeLocalFile()
    {
        ...
        $this->assertTrue($this->sftp->put($filename, $tmp_filename, SFTP::SOURCE_LOCAL_FILE), 'Failed asserting that local file could be successfully put().');
        $this->assertSame(128 * 1024 * 1024, $this->sftp->size($filename), 'Failed asserting that uploaded local file has the expected length.');
    }
}